...
def build_prompt(stats, session_dates):
    ...
    Prompt size is O(number_of_topics) — never grows with raw history length.
    """
    days_away = days_since_last_session(session_dates)
    today = datetime.now().strftime("%Y-%m-%d")

    # Topic summary sorted weakest-first, with recency indicator
    hist_lines = []
...
{review_note}{pre_exam_note}Статистика ученика по темам (накопленная за всё время):
{hist_summary}

Приоритеты при подборе тем для ЭТОГО квиза:
- 🔴 Темы ниже 60% → 40% вопросов (слабые места, приоритет)
- 🟡 Темы 60-85%   → 30% вопросов (закрепление)
- 🟢 Темы выше 85% → 15% вопросов (поддержание)
- Темы которых нет в истории → 15% вопросов (новый материал)
...
def generate_questions(stats, session_dates):
    ...
    prompt = build_prompt(stats, session_dates)
...
async def show_stats(message):
    ...
    if stats:
        weak   = sorted([(t, round(s["correct"]/s["total"]*100))
                         for t, s in stats.items() if s["total"] >= 3
